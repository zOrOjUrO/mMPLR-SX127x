...
class LoRaBeacon(LoRa):
    ...
    def chopNsendData(self, data):
        leng = len(data)
        i = 0
        for i in range(leng//PAYLD_SIZE):
            rawinput = data[i*PAYLD_SIZE: (i*PAYLD_SIZE)+PAYLD_SIZE]
            print(rawinput)
            formattedInput = [int(hex(ord(c)), 0) for c in rawinput]
            #self.write_payload([0x0f])
            self.write_payload(formattedInput)
            BOARD.led_on()
            self.set_mode(MODE.TX)
            sleep(1)
        if leng%PAYLD_SIZE != 0:
            #print(data[(leng//PAYLD_SIZE) * PAYLD_SIZE:])
            rawinput = data[(leng//PAYLD_SIZE) * PAYLD_SIZE:]
            print(rawinput)
            formattedInput = [int(hex(ord(c)), 0) for c in rawinput]
            #self.write_payload([0x0f])
            self.write_payload(formattedInput)
